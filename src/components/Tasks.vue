...
export default {
    methods: {
        ...
        toggleTaskCompletion(task) {
            this.completeTask(task).then(() => this.fetchTasks());
            // Toggle completion state logic if not handled in Vuex
        },
    },
                ...
                <div class="task-header">
                    <div class="task-buttons">
                        ...
                        <button @click="toggleTaskCompletion(task)">
                            {{ task.completed ? 'OpenTask' : 'Complete' }}
                        </button>
                        <h2>{{ task.title }}</h2>
                        <p>{{ task.description?.length > 50 ? task.description.substring(0, 47) + "..." : task.description }}
                        </p>
                    </div>
                </div>
                <div class="task-body">
...
.task-card {
    ...
    padding: 20px;
    width: 300px;
    transition: transform 0.2s, margin-left 0.5s ease;
}
